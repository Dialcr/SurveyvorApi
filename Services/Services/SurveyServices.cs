...
public class SurveyServices(EntityDbContext context) : CustomServiceBase(context)
{
    ...
    public async Task<
        OneOf<ResponseErrorDto, ICollection<SurveyOutputDto>>
    > SurveyActiveByUniversityId(int universityId)
    {
        var organization = await _context.University.SingleOrDefaultAsync(x =>
            x.Id == universityId
        );
    ...
    public OneOf<
        ResponseErrorDto,
        ICollection<ResponsePosibility>
    > SurveyResponsePosibilityBySurvey(int surveyAskId)
    {
        var responsePosibilities = _context
            .ResponsePosibilities.Where(x => x.SuveryAskId == surveyAskId)
            .ToList();
        if (!responsePosibilities.Any())
        {
            return new ResponseErrorDto()
            {
                ErrorCode = 404,
                ErrorMessage =
                    $"Responose posibility list of survey ask with id {surveyAskId} not found"
            };
        }
        return responsePosibilities;
    }

    public OneOf<ResponseErrorDto, int> CountSurveyResponsesBysurveyask(
        int surveyAskId,
        int reponse
    )
    {
        ...
        if (cant == 0)
        {
            return new ResponseErrorDto()
            {
                ErrorCode = 404,
                ErrorMessage =
                    $"not one response with reponse {reponse} for survey ask with id {surveyAskId}"
            };
        }
        return cant;
    }

    public OneOf<ResponseErrorDto, double> PorcentSurveyResponsesBySurveyask(
        int surveyAskId,
        int reponse
    )
    {
    ...
    > GetAllResponseBySurveyAskDescription(int surveyId, string surveyAskDescription)
    {
        ...
        if (!surveyResponses.Any())
        {
            return new ResponseErrorDto()
            {
                ErrorCode = 404,
                ErrorMessage =
                    $"not one response from survey question {surveyAskDescription} for survey with id {surveyId}"
            };
        }
    ...
    public OneOf<ResponseErrorDto, double> PorcentSurveyResponsesBySurveyaskDescription(
        int surveyId,
        string surveyAskDescription,
        string reponse
    )
    {
        var cant = _context.SurveyAskResponses.Count(x =>
    ...
    public async Task<OneOf<ResponseErrorDto, ICollection<SurveyOutputDto>>> SurveyByUniversityName(
        string universityName
    )
    {
        var organization = await _context.University.SingleOrDefaultAsync(x =>
            x.Name == universityName
        );
        if (organization is null)
        {
            return new ResponseErrorDto()
    ...
    public async Task<OneOf<ResponseErrorDto, SurveyOutputDto>> ApplicateSurveyAsync(
        SurveyInputDto surveyInputDto
    )
    {
        ...
        foreach (var question in surveyInputDto.Questions!)
        {
            var surveyAsk = new SurveyAsk
            {
                ...
                ResponsePosibilities =
                    question
                        .Answers?.Select(answer => new ResponsePosibility
                        {
    ...
    public IEnumerable<ApplicationOutputDto> GetAllApplicationsSurveys()
    {
        var applications = _context
            .Applications.Include(x => x.Survey)
    ...
    public async Task<OneOf<ResponseErrorDto, ApplicationOutputDto>> RespondApplicationsSurveyAsync(
        int applicationId,
        bool accepted
    )
    {
        var application = context
            ...
            .FirstOrDefault(x =>
                x.Id == applicationId && x.ApplicationState == ApplicationState.Pending
            );
    ...
    public IEnumerable<ApplicationOutputDto> GetAllRejectedApplicationsSurveys(int organizationId)
    {
        var applications = _context
            ...
            .ThenInclude(x => x.ResponsePosibilities)
            .Where(x =>
                x.ApplicationState == ApplicationState.Rejected
                && x.Survey!.OrganizationId == organizationId
            )
            .Select(x => x.ToApplicationOutputDto());
    ...
    public async Task<OneOf<ResponseErrorDto, SurveyResponsesDto>> AddResponseToSurvey(
        SurveyResponsesDto surveyResponsesDto
    )
    {
        var surveyQuestions = _context.SurveyAsks.Where(x =>
            x.SurveyId == surveyResponsesDto.SurveyId
        );
        ...
        {
            return new ResponseErrorDto() { ErrorCode = 500, ErrorMessage = e.Message };
        }
