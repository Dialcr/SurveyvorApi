...
public class SurveyController(
    ...
    ImportDbServices importDbServices,
    TokenUtil tokenUtil,
...
) : ControllerBase
{
    ...
    [ProducesResponseType(typeof(ResponseErrorDto), StatusCodes.Status400BadRequest)]
    [Authorize(Roles = "ORGANIZATION")]
    public IActionResult PorcentSurveyResponsesBySurveyaskDescription(
        int surveyId,
        string surveyAskDescription,
        string reponse
    )
    {
        var result = surveyServices.PorcentSurveyResponsesBySurveyaskDescription(
            surveyId,
            surveyAskDescription,
            reponse
        );
        if (result.TryPickT0(out var error, out var response))
        {
    ...
    {
        var result = surveyServices.GetAllResponseBySurveyAskDescription(
            surveyId,
            surveyAskDescription
        );
        if (result.TryPickT0(out var error, out var response))
    ...
    [ProducesResponseType(typeof(ResponseErrorDto), StatusCodes.Status400BadRequest)]
    [Authorize(Roles = "ADMIN")]
    public async Task<IActionResult> RespondApplicationsSurvey(
        RespondApplicationDto respondApplicationDto
    )
    {
        var result = await surveyServices.RespondApplicationsSurveyAsync(
            respondApplicationDto.ApplicationId,
    ...
    [ProducesResponseType(typeof(ResponseErrorDto), StatusCodes.Status400BadRequest)]
    [AllowAnonymous]
    public async Task<IActionResult> AddResponseToSurvey(
        [FromBody] SurveyResponsesDto surveyResponsesDto
    )
    {
