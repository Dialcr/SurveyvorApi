...
{
    public ImportDbServices(EntityDbContext context)
        : base(context) { }

    public async Task<OneOf<ResponseErrorDto, string>> ImportData(
        int organization,
        string descriptionOfSurvey,
        string filePath
    )
    {
        ...
        ICollection<Dictionary<string, string>> keyValuePairs =
        ...
        foreach (var dictionaryValue in keyValuePairs)
        {
            ...
            {
                string value =
                    $"{dictionaryValue.ElementAt(i).Key}: {dictionaryValue.ElementAt(i).Value}";
                string pattern = @"^(.*?\?) \[(.*?)\]: (.*)$";
                ...
                if (surveyAsk.IsNullOrEmpty())
                {
                    ...
                    !surveyAsk.Equals(match.Groups[1].Value.Trim())
                    || i == dictionaryValue.Count() - 1
                )
                {
                    ...
                    {
                        _context.ResponsePosibilities.Add(
                            new ResponsePosibility()
                            {
                    ...
                    var findSurveyAsk = _context.SurveyAsks.FirstOrDefault(x =>
                        x.SurveyId == findSurvey!.Id && x.Description.Equals(surveyAsk)
                    );
                    var surveyResponse = _context.ResponsePosibilities.FirstOrDefault(x =>
                        x.SuveryAskId == findSurveyAsk!.Id && x.ResponseValue == suerveyAskResponse
                    );
    ...
    static void ExploreJsonElement(
    ...
    */
        ...
        while (filesPending.Count > 0)
        {
            ...
            foreach (var jsonFile in jsonFiles)
            {
                try
                {
                    ...
                    var survey = _context.Surveys.FirstOrDefault(x =>
                        x.Description == Path.GetFileNameWithoutExtension(jsonFile)
                    );
                    if (survey is null)
                    {
                        var importData = await ImportData2(
                            organization.Id,
    ...
    )
    {
        ...
        {
            string value =
                $"{keyValuePairs.ElementAt(0).ElementAt(i).Key}: {keyValuePairs.ElementAt(0).ElementAt(i).Value}";
            string pattern = @"^(.*?\?) \[(.*?)\]: (.*)$";
        ...
        foreach (var dictionaryValue in keyValuePairs)
        {
            ...
            for (int i = 0; i < dictionaryValue.Count(); i++)
            {
                ...
                if (match.Groups[3].Value.Trim().ToLower().Equals("sí"))
                {
                    ...
                    var responseValue = ask.ResponsePosibilities.FirstOrDefault(x =>
                        x.ResponseValue.Equals(suerveyAskResponse)
                    );
